"""Optional Whisper transcription node for Raon-OpenTTS reference text."""
...
def get_whisper_pipeline(model_name: str, dtype: str, download_if_missing: bool):
    ...
    if cached is not None:
        patcher = getattr(cached, "_raon_opentts_aimdo_patcher", None)
    ...
    try:
        ...
        setattr(pipe, "_raon_opentts_aimdo_patcher", patcher)
        try:
...
class RaonWhisperTranscribe:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "audio": ("AUDIO", {"tooltip": "Reference audio to transcribe for the Raon-OpenTTS ref_text input."}),
                "model": (
    ...
    DESCRIPTION = "Transcribe reference AUDIO with Whisper for Raon-OpenTTS voice cloning. Audio output is the input passed through unchanged."
